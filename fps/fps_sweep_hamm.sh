...
for N in 32 64 128 256 384 400 432 450 480 486 500 512 540 576 600 640 648 675 720 729 750 768 800 810 864 900 960 972 1000 1024 1536 2048 3072 4096 8192 9216; do
    LOG="fps_N${N}.log"
    echo "Running N=${N} ..."

    # Run and capture full output (run from fps/, so PYTHONPATH=..)
    PYTHONPATH=.. PYTHONUNBUFFERED=1 uv run python -u -m scipyturbo.turbo_simulator_cufft "$N" 1e4 10 101 2>&1 | stdbuf -oL -eL tee -a "$LOG"
